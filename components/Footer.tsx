...
export default function Footer({ onNavigate }: FooterProps) {
  ...
  return (
    <footer className="bg-brand-page pt-24 pb-12 border-t border-white/5 relative z-10">
      <div className="max-w-7xl mx-auto px-4">

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div className="max-w-xs">
            <div className="flex items-center gap-2 mb-8">
              <span className="material-symbols-outlined text-brand-secondary text-3xl">token</span>
            ...
            <div className="flex gap-4">
              {['instagram', 'linkedin', 'x'].map((social) => (
                <button key={social} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-white hover:text-black transition-all border border-white/10">
                  <span className="material-symbols-outlined text-sm">{social === 'x' ? 'close' : social === 'instagram' ? 'photo_camera' : 'group'}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <h4 className="font-black text-white mb-8 text-[10px] uppercase tracking-[0.4em]">Products</h4>
            <ul className="space-y-4 text-gray-500 font-bold text-[10px] uppercase tracking-widest">
              <li><button onClick={() => navigate('listing', { category: 'Laptop' })} className="hover:text-white transition-all">Laptops</button></li>
        ...
        <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="flex flex-wrap gap-8">
            <div className="flex items-center gap-4 group">
              <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 group-hover:text-brand-secondary transition-colors">
