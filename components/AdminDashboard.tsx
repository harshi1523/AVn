    ...
    <div className="bg-brand-page h-screen flex overflow-hidden font-display">
      ...
      {isMobileMenuOpen && (
        <div
          ...
          className="lg:hidden fixed inset-0 bg-black/80 z-[50] backdrop-blur-md animate-in fade-in duration-300"
        />
      )}
                ...
                <div className="flex flex-wrap items-center gap-4">
                  {dateRange === 'custom' && (
                    <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-2xl px-3 py-1 shadow-inner">
                      <input
                        ...
                        className="bg-transparent text-white/50 text-[10px] font-bold uppercase p-2 focus:outline-none focus:text-white transition-colors"
                        placeholder="Start"
                      />
                  ...
                  <div className="relative group">
                    <select
                      ...
                      className="bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white/70 px-6 py-4 focus:outline-none focus:border-brand-primary cursor-pointer hover:bg-white/10 transition-all appearance-none pr-12 min-w-[180px]"
                    >
                      <option value="7d" className="bg-dark-sidebar text-white">Last 7 Days</option>
                      <option value="30d" className="bg-dark-sidebar text-white">Last 30 Days</option>
                      <option value="3m" className="bg-dark-sidebar text-white">Last 3 Months</option>
                      <option value="6m" className="bg-dark-sidebar text-white">Last 6 Months</option>
                      <option value="12m" className="bg-dark-sidebar text-white">Last 12 Months</option>
                      <option value="custom" className="bg-dark-sidebar text-white">Custom Range</option>
                    </select>
                    <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none group-hover:text-white/60 transition-colors">expand_more</span>
                  </div>
                </div>
              )}
            </header>
            ...
            {activeTab === 'overview' && (
              <div className="space-y-12 pb-20">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-8">
                  {stats.map((stat, i) => (
                    <div key={i} className="group glass-card p-8 rounded-[2.5rem] shadow-elevated hover:scale-[1.02] hover:bg-white/[0.05] transition-all duration-500 relative overflow-hidden" data-testid={`stat-card-${stat.label.toLowerCase().replace(/\s+/g, '-')}`}>
                      ...
                      <div className="flex items-end gap-2">
                        <p className="text-3xl lg:text-4xl font-black text-white tracking-tighter" data-testid={`stat-value-${stat.label.toLowerCase().replace(/\s+/g, '-')}`}>{stat.value}</p>
                      </div>
                    </div>
                  ))}
                </div>

                <div className="glass-card rounded-[3rem] p-8 lg:p-14 shadow-elevated relative overflow-hidden mt-12 mb-12" data-testid="sales-chart-container">
                    ...
                    <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pr-8">
                      {[...Array(5)].map((_, i) => (
                        <div key={i} className="w-full h-px bg-white/[0.03] relative">
                          <span className="absolute -right-10 -top-2 text-[8px] font-black text-gray-600">
                            {i === 4 ? '0' : ''}
                          </span>
                        </div>
                      ))}
                        ...
                        <>
                          ...
                          <div className="flex justify-between mt-10 px-2 overflow-x-auto no-scrollbar">
                            {chartData.filter((_, i) => {
                              if (chartData.length > 15) return i % Math.ceil(chartData.length / 10) === 0;
                              return true;
                            }).map((data, i) => (
                      ...
                      <div className="flex gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            {viewingUser.kycStatus === 'approved' && viewingUser.kycVerifiedDate && (
                              <span className="px-2 py-0.5 rounded-full text-[9px] font-black uppercase border bg-brand-primary/10 text-brand-primary border-brand-primary/20">
                                Expires: {(() => {
                                  const d = new Date(viewingUser.kycVerifiedDate);
